...
/* ── Video list tab (used for projects with multiple videos) ── */
function VideoList({ videos }: { videos: NonNullable<Project["videos"]> }) {
  return (
        ...
        {videos.map((v, i) => (
          <li key={i}>
            <a
              ...
              className="flex items-center justify-between gap-3 px-3.5 py-2.5 hover:bg-[var(--bg2)] transition-colors group"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs text-[var(--fg2)] truncate group-hover:text-[var(--blue)] transition-colors">
                  {v.title}
                </span>
              </div>
              <ArrowUpRight size={11} className="text-[var(--subtle)] shrink-0 group-hover:text-[var(--blue)] transition-colors" />
            </a>
          </li>
        ))}
      </ul>
...
export default function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  const thumbnailUrl = project.thumbnail ?? (firstVideoUrl ? ytThumb(firstVideoUrl) : null);
    ...
    >
      ...
      {thumbnailUrl && (
        <div className="relative w-full overflow-hidden border-b border-[var(--border)]"
          ...
          {/* Main image — fallback from maxres → hq if not found */}
          <img
            src={thumbnailUrl}
          ...
          />
          {/* Dark-to-transparent gradient at bottom edge */}
          ...
          <div className="absolute top-2.5 left-3 flex items-center gap-1.5">
            ...
            {project.featured && (
              <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded"
                style={{ background: "rgba(37,99,235,0.85)", color: "#fff", backdropFilter: "blur(4px)" }}>
                Featured
              </span>
            )}
          </div>

          {/* Play-count icon hint bottom-right */}
          {hasVideos && (
      ...
      <div className="flex flex-col p-6 flex-1">
        ...
        {project.metrics && project.metrics.length > 0 && (
          <div className="space-y-1.5 mb-4 p-3.5 rounded-md border border-[var(--border2)]"
            style={{ background: "var(--bg2)" }}>
        ...
        {hasVideos && (
          <div className="mb-4">
            ...
            >
              ...
              <span className="text-[var(--subtle)] font-normal ml-0.5">
                {showVideos ? "▲" : "▼"}
              </span>
            </button>
        ...
        <div className="pt-4 border-t border-[var(--border)] flex items-center justify-between">
          ...
          {project.client && (
            <span className="text-xs text-[var(--subtle)]">{project.client}</span>
          )}
        </div>
      </div>
    </motion.article>
